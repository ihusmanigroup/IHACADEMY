  ...
  if (!insideTemplate) {
    ...
    for (let j = 0; j < line.length; j++) {
      if (line.charCodeAt(j) === 96) { // `
        if (j === 0 || line.charCodeAt(j - 1) !== 92) { // not preceded by \
    ...
    if (unescapedBacktickCount === 1 && lastUnecscapedIdx > 0) {
      // This line has exactly one unescaped backtick - it's the closing backtick
      ...
      // Multiple unescaped backticks - escape all but the last one
      let escaped = '';
      let unescapedSeen = 0;
      for (let j = 0; j < line.length; j++) {
      ...
      // No unescaped backticks, or only escaped ones
      output.push(line);
    }
  }
}
...
console.log('Fixed backticks. Modified', fixedCount, 'lines inside template literals');
