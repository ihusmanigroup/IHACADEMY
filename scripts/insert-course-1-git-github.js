 ...
 * Loads rich 500+ word topic content from course-data/course-1.json
 *
 * Usage:
...
async function run() {
  ...
  const coursePath = path.join(__dirname, '..', 'course-data', 'course-1.json')
  if (!fs.existsSync(coursePath)) {
    console.error('  ❌ course-data/course-1.json not found. Run node scripts/generate-course-data.js first.')
    process.exit(1)
  }
    ...
    try {
      ...
      const courseResult = await client.query(
        ...
        [
          ...
          courseData.lessons.length, courseData.xp_reward || 50,
