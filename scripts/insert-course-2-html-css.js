/**
 * IH Academy — Course 2: HTML & CSS Basics
 * Loads rich 500+ word topic content from course-data/course-2.json
 *
 * Usage:
 *   node scripts/insert-course-2-html-css.js
 */
...
async function run() {
  ...
  const coursePath = path.join(__dirname, '..', 'course-data', 'course-2.json')
  if (!fs.existsSync(coursePath)) {
    console.error('  ❌ course-data/course-2.json not found. Run node scripts/generate-course-data.js first.')
    process.exit(1)
  }
    ...
    try {
      ...
      const courseResult = await client.query(
        ...
        [
          ...
          courseData.lessons.length, courseData.xp_reward || 40,
