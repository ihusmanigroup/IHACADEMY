/**
 * IH Academy — Course 5: Full-Stack Web Development & Databases
 * Loads rich 500+ word topic content from course-data/course-5.json
 *
 * Usage:
 *   node scripts/insert-course-5-databases.js
 */
...
async function run() {
  ...
  const coursePath = path.join(__dirname, '..', 'course-data', 'course-5.json')
  if (!fs.existsSync(coursePath)) {
    console.error('  ❌ course-data/course-5.json not found.')
    process.exit(1)
  }
    ...
    try {
      ...
      const courseResult = await client.query(
        ...
        [
          ...
          courseData.lessons.length, courseData.xp_reward || 75,
          courseData.price ?? 0, true, false
        ]
      )
